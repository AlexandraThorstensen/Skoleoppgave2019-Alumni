...
try {
    if (isset($_POST['oppdaterpersonalia'])) {
        ...
        if($q->rowCount() == 0) {
                $sql = "INSERT INTO brukerprofil (idbruker, fornavn, etternavn, bosted) VALUES (?,?,?,?)";
                $stmt = $db->prepare($sql);
                $stmt->bindValue(1, $brukerid, PDO::PARAM_INT);
                $stmt->bindValue(2, $_POST['fornavn'], PDO::PARAM_STR);
                $stmt->bindValue(3, $_POST['etternavn'], PDO::PARAM_STR);
                $stmt->bindValue(4, $_POST['bosted'], PDO::PARAM_STR);
                $stmt->execute(); 
                header("Location: minprofilback.php?melding=Personalia er oppdatert!");
        }
            ...
            if($_POST['fornavn']) { 
                ...
                $stmt->execute(); 
                header("Location: minprofilback.php?melding=Fornavn er oppdatert!");
            }
            
            else if($_POST['etternavn']) { 
                $sql = "UPDATE brukerprofil SET etternavn=? WHERE idbruker=?";
                $stmt = $db->prepare($sql);
                $stmt->bindValue(1, $_POST['etternavn'], PDO::PARAM_STR);
                $stmt->bindValue(2, $brukerid, PDO::PARAM_INT);
                $stmt->execute(); 
                header("Location: minprofilback.php?melding=Etternavn er oppdatert!");
            }
            
            else if($_POST['bosted']) { 
                $sql = "UPDATE brukerprofil SET bosted=? WHERE idbruker=?";
                $stmt = $db->prepare($sql);
                $stmt->bindValue(1, $_POST['bosted'], PDO::PARAM_STR);
                $stmt->bindValue(2, $brukerid, PDO::PARAM_INT);
                $stmt->execute(); 
                header("Location: minprofilback.php?melding=Bosted er oppdatert!");
            }
            
            else { 
                $sql = "UPDATE brukerprofil SET fornavn=?, etternavn=?, bosted=? WHERE idbruker=?";
                $stmt = $db->prepare($sql);
                $stmt->bindValue(1, $_POST['fornavn'], PDO::PARAM_STR);
                $stmt->bindValue(2, $_POST['etternavn'], PDO::PARAM_STR);
                $stmt->bindValue(3, $_POST['bosted'], PDO::PARAM_STR);
                $stmt->bindValue(4, $brukerid, PDO::PARAM_INT);
                $stmt->execute(); 
                header("Location: minprofilback.php?melding=Personalia er oppdatert!");
            }
        }
    ...
    if (isset($_POST['oppdaterstudie'])) {
        ...
        if($q->rowCount() == 1) { 
            $sql = "UPDATE brukerstudie SET idstudie=?, kull=? WHERE idbruker=?";
            $stmt = $db->prepare($sql);
            ...
            $stmt->execute(); 
            header("Location: minprofilback.php?melding=Studie er oppdatert!");
        }
    ...
    if (isset($_POST['byttpassord'])) {
        if ($_POST['nyttpassord'] == $_POST['bekreftnyttpassord']) {
            ...
            header("Location: minprofilback.php?melding=Ditt passord er endret!");
        }
        ...
        if ($sth->execute(array($_POST['gammelepost']))) {
            if ($row = $sth->fetchAll()) {
                if ($_POST['nyepost'] == $_POST['bekreftnyepost']) {
                    ...
                    $db->exec($sql);
                    header("Location: minprofilback.php?melding=Din epostadresse har blitt endret!");
                }
...
<head>
    ...
    <title>Rediger personalia</title>
...
<section class="redigerpersonalia">
	<article class="byttpersonalia">
        <h1>Personalia</h1>
            <form method="post" action="redigerpersonaliaback.php">
    ...
    <article class="byttpassord">
        <h1>Bytt passord</h1>
            <form method="post" name="byttpassord" action="redigerpersonaliaback.php">
    ...
    <article class="byttepost">
        <h1>Bytt epost</h1>
            <form method="post" name="byttepost" action="redigerpersonaliaback.php">
